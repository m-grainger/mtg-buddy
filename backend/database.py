...
def db_error_handler(error, func_name):
    print(
        f"Error was thrown from {func_name} and resulted in error of {error}")
...
async def fetch_one_todo(title):
    try:
        ...
        tb_str = traceback.extract_tb(e.__traceback__).as_list()
        func_name = tb_str[-1][2]
        db_error_handler(e, func_name)
